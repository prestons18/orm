...
async fn test_transaction_commit() -> Result<()> {
    ...
    // Insert initial data
    backend.execute_raw("INSERT INTO accounts (name, balance) VALUES ('Alice', 100)").await?;
    backend.execute_raw("INSERT INTO accounts (name, balance) VALUES ('Bob', 50)").await?;

    // Start a transaction
    let mut tx = db.begin_transaction().await?;

    // Perform operations within transaction
    tx.execute("UPDATE accounts SET balance = balance - 30 WHERE name = 'Alice'").await?;
    tx.execute("UPDATE accounts SET balance = balance + 30 WHERE name = 'Bob'").await?;

    // Commit the transaction
    tx.commit().await?;

    // Verify changes were committed
    let results = backend.fetch_all("SELECT name, balance FROM accounts ORDER BY name").await?;
...
async fn test_transaction_auto_rollback_on_drop() -> Result<()> {
...

    {
        ...
        let mut tx = db.begin_transaction().await?;
        tx.execute("UPDATE accounts SET balance = 200 WHERE name = 'Alice'").await?;
        // Transaction dropped without commit - should auto-rollback
    }

    // Verify changes were rolled back
    let results = backend.fetch_all("SELECT balance FROM accounts WHERE name = 'Alice'").await?;
    let alice_balance = results[0].get("balance").and_then(|v| v.as_i64()).unwrap();
...
async fn test_transaction_fetch_operations() -> Result<()> {
    ...
    // Insert initial data
    backend.execute_raw("INSERT INTO users (name, age) VALUES ('Alice', 30)").await?;
    backend.execute_raw("INSERT INTO users (name, age) VALUES ('Bob', 25)").await?;

    // Start a transaction
    let mut tx = db.begin_transaction().await?;

    // Insert new user within transaction
    tx.execute("INSERT INTO users (name, age) VALUES ('Charlie', 35)").await?;

    // Fetch all users within transaction (should see the new user)
    let users = tx.fetch_all("SELECT name, age FROM users ORDER BY name").await?;
    assert_eq!(users.len(), 3);

    // Fetch one user within transaction
    let alice = tx.fetch_one("SELECT name, age FROM users WHERE name = 'Alice'").await?;
    ...
    let final_users = backend.fetch_all("SELECT name FROM users ORDER BY name").await?;
...
async fn test_transaction_isolation() -> Result<()> {
    ...
    // Insert initial data
    backend.execute_raw("INSERT INTO counters (value) VALUES (0)").await?;

    // Start a transaction
    let mut tx = db.begin_transaction().await?;

    // Update within transaction
    tx.execute("UPDATE counters SET value = 100 WHERE id = 1").await?;

    // Read from outside the transaction (should see old value due to isolation)
    let outside_result = backend.fetch_one("SELECT value FROM counters WHERE id = 1").await?;
    ...
    let inside_result = tx.fetch_one("SELECT value FROM counters WHERE id = 1").await?;
    ...
    let final_result = backend.fetch_one("SELECT value FROM counters WHERE id = 1").await?;
...
async fn test_transaction_error_handling() -> Result<()> {
    ...
    // This should succeed
    tx.execute("INSERT INTO test (id, value) VALUES (1, 'test')").await?;
    
    // This should fail (duplicate primary key)
    let result = tx.execute("INSERT INTO test (id, value) VALUES (1, 'duplicate')").await;
    assert!(result.is_err());

    // Transaction should still be usable after error
    tx.execute("INSERT INTO test (id, value) VALUES (2, 'test2')").await?;
